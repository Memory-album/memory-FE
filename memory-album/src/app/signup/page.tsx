...
const signup = ({ slides, options }: PropType) => {
  ...
  useEffect(() => {
    ...
    if (!nameInput || !emailInput || !passwordInput || !verifyPasswordInput) {
    ...
    return () => {
      ...
      verifyPasswordInput.removeEventListener('input', validateForm);
    };
  }, []);
  ...
  const handleVerifyEmailCode = async () => {
    try {
      ...
      setEmailVerify(true);
      console.log('Signup success', userData, response);
    } catch (error) {
      console.error('Signup failed', error);
      alert('코드가 유효하지 않습니다.');
    }
